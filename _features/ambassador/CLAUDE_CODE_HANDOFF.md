...
30. **PL/pgSQL stored function lint check — catch variable/column collisions at create time (Slice 6B-2 hotfix retro, 2026-04-25).** The original `create_payout_batch()` (commit `238423a`) shipped with a `RETURNS TABLE(... currency text)` output column that shadowed bare `SELECT currency FROM model_listing_payments` references inside the homogeneity subquery. Postgres's `plpgsql.variable_conflict = error` (default) raised 42702 at first-invocation time during direct SQL Editor verification — atomic rollback worked, but the bug would have stopped the first real Wednesday batch in production. Hot-fixed in `49bb9e5`. The cheap manual control (Critical implementation patterns §11 added in the same fix commit) is "prefix locals with `v_`/`_` AND avoid RETURNS TABLE output names that collide with columns being read." A stronger control would be CI tooling that catches this at function-create time. **Candidates:** (a) `plpgsql_check` extension — runs static analysis over PL/pgSQL bodies and surfaces variable_conflict + uninitialized-variable + dead-code issues; (b) custom GitHub Action that does `psql -c "CREATE OR REPLACE FUNCTION ..." ; psql -c "SELECT * FROM <fn>(...);"` against a throwaway DB to dry-run each migration before merge; (c) lighter: Supabase CLI `supabase db lint` if it gains plpgsql-aware checks. **Action:** evaluate `plpgsql_check` first (cheapest install, well-maintained); if it requires a Postgres extension that Supabase managed Postgres doesn't allow, fall back to (b) GitHub Action. Not a launch blocker — V1 has 2 PL/pgSQL functions (`claim_wish_for_payment` + `create_payout_batch`), small surface area; manual review per the new §11 convention catches the known classes. Worth doing before Slice 7's payout-paid notification trigger or any future stored-function additions.
31. **Audit existing PL/pgSQL stored functions for variable/column collisions (Slice 6B-2 hotfix retro, 2026-04-26).** Pair with Critical implementation patterns §11 (added in `49bb9e5`). The §11 convention catches the trap going forward; this item is the **retroactive audit** of functions already shipped — `claim_wish_for_payment(p_wish_id uuid, p_lock_minutes int)`, `revert_expired_wish_locks()`, `get_top_click_categories(...)` (Slice 4D), `cleanup_phantom_auth_users()` (Slice 1.5), and any others that surface via `SELECT proname FROM pg_proc WHERE prolang = (SELECT oid FROM pg_language WHERE lanname='plpgsql')`. **Audit checklist per function:** (a) list every DECLARE-block local — flag any not prefixed with `v_` or `_`; (b) if the function uses RETURNS TABLE, list every output column name + cross-reference against the columns the function body reads via SELECT/UPDATE/INSERT; (c) for any collision found, flag whether the function has SELECT-list bare references to that column (the actual trap path) versus only WHERE-clause refs (usually safe). **Rename strategy:** locals get `v_` prefix; RETURNS TABLE output columns get table-qualification + alias on bare refs (matches the 49bb9e5 hotfix pattern), never rename the RETURNS TABLE column itself if any caller relies on the property name. **Risk if deferred:** silent until first invocation in production, atomic rollback saves data integrity but service incident occurs. **Cheap implementation:** dedicated hardening commit running through the function list + each one re-deployed via `CREATE OR REPLACE`. Estimate: 1 hour total. Worth doing before Slice 7's payout-paid notification trigger or any other stored-function additions land. Item 30 (tooling) is the durable control; this is the one-time backfill.
