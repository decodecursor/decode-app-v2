    ...
    const handleVerification = async () => {
      try {
        ...
        if (!token && typeof window !== 'undefined') {
          ...
          // Check for error in hash (verification failed)
          const error = hashParams.get('error')
          const errorDescription = hashParams.get('error_description')

          if (error) {
            console.error('❌ Verification error from hash:', error, errorDescription)
          ...
          if (accessToken && refreshToken) {
            console.log('✅ Email change verified - session tokens received in hash')
            ...
            try {
              ...
              if (sessionError) {
                ...
                setMessage('Verification succeeded but session error occurred. Please try logging in.')
                setLoading(false)
                return
              }

              console.log('✅ Session set successfully from email change verification')
              setSuccess(true)
              ...
              setMessage('Verification succeeded but failed to set session. Please try logging in.')
              setLoading(false)
              return
            }
          }
