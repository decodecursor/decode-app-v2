...
export default function BankAccountPage() {
  ...
  const checkStripeAccount = async (userId: string) => {
    try {
      ...
      const { data: userData, error } = await supabase
        .from('users')
        .select('email, professional_center_name, stripe_connect_account_id, stripe_connect_status, stripe_onboarding_completed')
        .eq('id', userId)
        .single()
      ...
      if (userData?.stripe_connect_account_id) {
        console.log('Found existing Stripe Connect account:', userData.stripe_connect_account_id)
        ...
        if (userData.stripe_onboarding_completed) {
          ...
          setMessage({ type: 'success', text: 'Your bank account is already connected and verified.' })
        } else {
          setCurrentStep('onboarding')
          setMessage({ type: 'info', text: 'Please complete your bank account setup.' })
        }
      } else {
        // No account exists, create new one
        console.log('No Stripe Connect account found, creating new one for:', userData?.email)
  ...
  const createStripeAccount = async () => {
    try {
      ...
      if (!user?.id) {
        setMessage({ type: 'error', text: 'User session not found. Please refresh the page.' })
        return
      }
      ...
      // Store the account ID in the database for future use
      ...
      if (updateError) {
        console.error('Failed to store Connect account ID:', updateError)
      }
  ...
  const initializeStripeConnect = async () => {
    try {
      // Ensure user and stripeAccountId are available
      if (!user?.id || !stripeAccountId) {
        setMessage({ type: 'error', text: 'Missing required information. Please refresh the page.' })
        return
      }
