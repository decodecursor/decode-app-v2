        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 payments-metrics-grid" style={{gridAutoRows: '1fr'}}>
          ...
          <div className="bg-white/5 rounded-lg p-4 payment-metric-card">
            {(userRole === USER_ROLES.ADMIN || userRole === 'Admin') ? (
              ...
              // Loading state - show line while determining role to prevent flash
              <div className="pt-4">
                <div className="w-16 h-1 bg-white/30 rounded-full"></div>
              </div>
            ) : (
          ...
          {getFilteredPayments().map((payment, index) => {
            ...
            return (
              <div key={`${payment.id}-${index}`} className="bg-gray-800/50 rounded-lg p-3 hover:bg-gray-700/50 transition-colors paid-paylink-card">
                ...
                {/* Desktop Layout */}
                <div className="hidden md:grid gap-x-4 items-center" style={{gridTemplateColumns: '1.6fr 1.2fr 1.4fr 1.6fr 1.0fr 1.1fr'}}>
    ...
    {showDatePicker ? (
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[10001]">
        <div className="cosmic-card max-w-2xl mx-4" style={{ height: '480px' }}>
          ...
          />
          <div className="grid grid-cols-2 gap-4" style={{ marginTop: '-40px', paddingTop: '10px' }}>
            <button
              onClick={() => setShowDatePicker(false)}
              className="cosmic-button-secondary py-2 px-4 border border-white/30 rounded-lg text-gray-400 hover:text-gray-300 transition-colors"
            >
              Cancel
            </button>
            <button
