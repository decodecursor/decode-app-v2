...
export function VideoUploadCountdown({ tokenExpiresAt, hasVideo, showAsFullStatus = false, asButton = false, auctionEnded = false, compactMobile = false }: VideoUploadCountdownProps) {
  ...
  if (showAsFullStatus && !tokenExpiresAt && !hasVideo) {
    ...
    return (
      <div className="flex items-center gap-1 md:gap-2 px-3 py-1 md:py-1.5 text-xs md:text-sm text-gray-400 border-0 rounded-lg" style={{ border: 'none' }}>
  ...
  if (showAsFullStatus) {
    ...
    if (compactMobile) {
      return (
        <div className="flex items-center gap-1 md:gap-2 px-3 py-1 md:py-1.5 text-xs md:text-sm border-0 rounded-lg">
          {/* Camera Icon */}
          <svg className={`flex-shrink-0 w-3.5 h-3.5 md:w-4 md:h-4 ${colorClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
          </svg>
    ...
    return (
      <div className="flex items-center gap-2 px-3 py-1.5 text-sm">
        {/* Camera Icon */}
        <svg className={`flex-shrink-0 w-4 h-4 ${colorClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
