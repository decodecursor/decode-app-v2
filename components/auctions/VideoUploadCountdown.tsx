...
export function VideoUploadCountdown({ tokenExpiresAt, hasVideo, showAsFullStatus = false, asButton = false, auctionEnded = false, compactMobile = false }: VideoUploadCountdownProps) {
  ...
  if (showAsFullStatus && !tokenExpiresAt && !hasVideo) {
    ...
    return (
      <div className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 text-xs md:text-sm text-gray-400 border-0 md:border md:border-white/30 rounded-lg">
  ...
  if (showAsFullStatus) {
    ...
    if (compactMobile) {
      return (
        <div className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 text-xs md:text-sm border-0 md:border md:border-white/30 rounded-lg">
          {/* Camera Icon */}
