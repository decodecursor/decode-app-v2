...
export function VideoPlayback({ auctionId, auction, className = '', onPayoutUnlocked }: VideoPlaybackProps) {
  ...
  const [winnerInstagram, setWinnerInstagram] = useState<string | null>(null);
  ...
  const fetchVideo = async () => {
    try {
      ...
      if (data.winner) {
        ...
        setWinnerInstagram(data.winner.instagram_username || null);
      }
      ...
      <div className="px-4 py-3 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            ...
            {winnerName && (
              <div className="mt-1">
                <p className="text-sm text-gray-700">{winnerName}</p>
                {winnerInstagram && (
                  <p className="text-xs text-gray-500 flex items-center gap-1">
                    <InstagramIcon className="w-3 h-3 text-pink-600" />
                    <a
                      href={`https://www.instagram.com/${winnerInstagram}`}
                      target="_blank"
      ...
      {expired ? (
        ...
        <div className="relative bg-black aspect-video">
          ...
          {!hasStartedPlaying && (
            ...
            >
              <div className="w-16 h-16 bg-white bg-opacity-90 rounded-full flex items-center justify-center group-hover:bg-opacity-100 group-hover:scale-110 transition-all">
      ...
      {!expired && isPayoutLocked && (
        <div className="px-4 py-3 border-t border-gray-200">
          <div className="flex items-center justify-center gap-2">
            <svg className="w-[15px] h-[15px] sm:w-5 sm:h-5 text-amber-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <span className="text-[11px] sm:text-sm font-medium text-amber-800">Watch the complete video to unlock your payout</span>
          </div>
        </div>
      )}
    </div>
