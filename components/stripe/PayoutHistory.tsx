...
export function PayoutHistory({ userId, userRole, onNewPayout, refreshTrigger }: PayoutHistoryProps) {
    ...
    try {
      ...
      const rows = (data || []).map(payout => {
        const baseRow = [
          ...
          `"${formatAmount(Number(payout.payout_amount_aed) || 0)}"`
        ]
  ...
  return (
    <div className="cosmic-card">
      <div className="flex items-center justify-between mb-6">
        ...
        {payouts.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:text-white bg-white/3 hover:bg-white/10 rounded-lg transition-colors"
          >
      ...
      {payouts.length === 0 ? (
        <p className="text-gray-300 text-center py-8">No payouts yet. Your first payout will appear here.</p>
      ) : (
        <>
          <div className="space-y-3">
                      ...
                      <span className="flex items-center justify-end gap-1 cosmic-label">
                        {payout.paid_at ? (
                          <>
                            ...
                            <span className="text-white">{formatDate(payout.paid_at)}</span>
                          </>
                        ) : (
                          <>
                            <span className="text-purple-400">Request:</span>
                            <span className="text-white">{formatDate(payout.created_at)}</span>
                          </>
                        )}
                      </span>
                    </div>
